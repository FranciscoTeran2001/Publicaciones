...
public class PaperService {
    ...
    @Autowired
    private NotificacionProducer notificacionProducer; // Inyectamos el producer
    ...
    public ResponseDto crearPaper(PaperDto dto) {
        ...
        notificacionProducer.enviarNotificacion(
                "Nuevo paper creado: " + paperGuardado.getTitulo(),
                "PAPER"
        );

        // 2. Enviar al catálogo
        notificacionProducer.enviarACatalogoPaper(paperGuardado);

        return new ResponseDto("Paper registrado exitosamente", paperGuardado);
    }
